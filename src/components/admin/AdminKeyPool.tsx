...
export default function AdminKeyPool() {
  ...
  const handleDelete = async (id: number) => {
    ...
    try { await deleteKey(id); await load(); } catch (e: unknown) { setError(getErrorMessage(e)); }
  };

  const handleToggle = async (id: number) => {
    try { await toggleKey(id); await load(); } catch (e: unknown) { setError(getErrorMessage(e)); }
  };
                  ...
                  <tr key={r.id} className="border-b border-gray-100">
                    ...
                    <td className="px-4 py-2">
                      <button onClick={async () => {
                        ...
                        try { await deleteRecharge(r.id); setRecharges(prev => prev.filter((x: any) => x.id !== r.id)); } catch (e: unknown) { setError(getErrorMessage(e)); }
