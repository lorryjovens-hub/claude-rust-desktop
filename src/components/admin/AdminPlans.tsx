...
export default function AdminPlans() {
  ...
  const load = useCallback(async () => {
    try { setPlans(await getPlans()); } catch (e: unknown) { setError(getErrorMessage(e)); }
  }, []);
  ...
  const handleDelete = async (id: number) => {
    ...
    try { await deletePlan(id); await load(); } catch (e: unknown) { setError(getErrorMessage(e)); }
  };

  const handleToggle = async (id: number) => {
    try { await togglePlan(id); await load(); } catch (e: unknown) { setError(getErrorMessage(e)); }
  };
