...
export default function AdminUsers() {
  ...
  const handleBan = async (u: User) => {
    ...
    try { await banUser(u.id); await load(); } catch (e: unknown) { setError(getErrorMessage(e)); }
  };

  const handleUnban = async (u: User) => {
    try { await unbanUser(u.id); await load(); } catch (e: unknown) { setError(getErrorMessage(e)); }
  };
