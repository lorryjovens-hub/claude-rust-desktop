...
import { ArrowLeft, Plus, Trash2, Play, Sparkles, Monitor, Smartphone, FileText, Video, Palette, Zap, Eye, Download, ExternalLink, Loader2, Send, X } from 'lucide-react';
...
const DESIGN_TYPES = [
  {
    id: 'prototype',
    label: '交互原型',
    description: '高保真 App/Web 原型，可点击交互',
    icon: Smartphone,
    color: '#8B5CF6',
    examples: ['iOS App 原型，4 个核心屏幕', 'SaaS Dashboard 原型'],
  },
  {
    id: 'slides',
    label: '演讲幻灯片',
    description: 'HTML 幻灯片 + 可编辑 PPTX 导出',
    icon: FileText,
    color: '#3B82F6',
    examples: ['AI 心理学演讲 PPT', '产品发布会幻灯片'],
  },
  {
    id: 'motion',
    label: '时间轴动画',
    description: 'MP4/GIF 动画，支持 60fps 插帧',
    icon: Video,
    color: '#EC4899',
    examples: ['产品发布 60 秒动画', '品牌 Logo 揭示动画'],
  },
  {
    id: 'infographic',
    label: '信息图/可视化',
    description: '印刷级排版，数据驱动可视化',
    icon: Palette,
    color: '#10B981',
    examples: ['年度报告信息图', '数据对比可视化'],
  },
  {
    id: 'review',
    label: '5 维专家评审',
    description: '哲学一致性/视觉层级/细节执行等',
    icon: Eye,
    color: '#F59E0B',
    examples: ['设计稿 5 维度评审', '品牌一致性检查'],
  },
];
...
const DesignPage = () => {
  ...
  const handleGenerate = async () => {
    ...
    // 这里应该调用后端 API 生成 HTML
    // 目前使用预设的 showcase 作为演示
    const showcaseMap: Record<string, string> = {
      'prototype': '/design-assets/showcases/website-homepage/homepage-build.html',
      'slides': '/design-assets/showcases/ppt/ppt-build.html',
      'motion': '/design-assets/showcases/cover/cover-build.html',
      'infographic': '/design-assets/showcases/infographic/infographic-build.html',
      'review': '/design-assets/showcases/website-saas/saas-build.html',
    };

    const completedProject: DesignProject = {
      ...project,
      status: 'completed',
      htmlContent: showcaseMap[selectedType] || showcaseMap['prototype'],
      previewUrl: showcaseMap[selectedType] || showcaseMap['prototype'],
    };
  ...
  const getTypeIcon = (type: string) => {
    const typeInfo = DESIGN_TYPES.find(t => t.id === type);
    if (!typeInfo) return Sparkles;
    return typeInfo.icon;
  };

  const getTypeColor = (type: string) => {
    const typeInfo = DESIGN_TYPES.find(t => t.id === type);
    return typeInfo?.color || '#8B5CF6';
  };
  ...
  if (previewProject) {
    return (
      <div className="flex-1 h-full bg-claude-bg flex flex-col">
        {/* Preview Header */}
        <div className="flex items-center justify-between px-4 py-3 border-b border-claude-border bg-claude-surface">
          <div className="flex items-center gap-3">
            <button
              onClick={() => setPreviewProject(null)}
              className="flex items-center gap-1.5 text-claude-textSecondary hover:text-claude-text transition-colors"
            >
              <ArrowLeft size={16} />
        ...
        <div className="flex-1 bg-claude-bg p-4">
          <div className="w-full h-full rounded-xl border border-claude-border overflow-hidden bg-white">
            {previewProject.htmlContent ? (
              <iframe
                ref={iframeRef}
                src={previewProject.htmlContent}
      ...
      <div className="max-w-[900px] mx-auto px-4 py-8 md:px-8 md:py-12">
        ...
        {showNewProject && (
          <div className="mb-8 p-5 bg-claude-input border border-claude-border rounded-xl">
            <h3 className="text-[15px] font-semibold text-claude-text mb-4">创建新设计</h3>
            ...
            <div className="mb-4">
              ...
              <div className="relative">
                <textarea
                  ...
                  placeholder="描述你想要的设计，例如：做一个 AI 番茄钟 iOS 原型，4 个核心屏幕要真能点击"
                  className="w-full px-3 py-2.5 bg-transparent border border-claude-border rounded-lg text-[14px] text-claude-text focus:outline-none focus:border-blue-500 resize-none"
                  rows={3}
                  onKeyDown={(e) => {
                ...
                <div className="flex flex-wrap gap-2">
                  {DESIGN_TYPES.find(t => t.id === selectedType)?.examples.map((ex, i) => (
                    <button
                      ...
                      className="px-2.5 py-1 bg-claude-hover text-claude-textSecondary hover:text-claude-text rounded-md text-[12px] transition-colors"
                    >
                      {ex}
                    </button>
                  ))}
            ...
            <div className="flex justify-end gap-3">
              <button
                onClick={() => { setShowNewProject(false); setInputPrompt(''); setSelectedType(''); setSelectedStyle(''); }}
              ...
              <button
                ...
                disabled={generating || !inputPrompt.trim() || !selectedType}
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mt-6 max-w-[600px]">
              {DESIGN_TYPES.map(type => {
                ...
                return (
                  <div
                    key={type.id}
                    className="p-3 bg-claude-input border border-claude-border rounded-lg text-center"
                  >
                    <div
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    <span className={`px-2 py-0.5 rounded text-[11px] font-medium ${
                      ...
                      project.status === 'generating' ? 'bg-yellow-500/10 text-yellow-400' :
                    ...
                    {project.status === 'completed' && (
                      <button
                        ...
                        className="px-2.5 py-1.5 text-[12px] font-medium text-claude-text hover:bg-claude-hover rounded-md transition-colors flex items-center gap-1"
                      >
                        <Play size={12} />
                        预览
                      </button>
                    ...
                    <button
                      ...
                      className="w-8 h-8 rounded-lg flex items-center justify-center text-claude-textSecondary hover:text-[#B9382C] hover:bg-[#B9382C]/10 transition-colors opacity-0 group-hover:opacity-100"
                    >
                      <Trash2 size={14} />
                    </button>
                  </div>
