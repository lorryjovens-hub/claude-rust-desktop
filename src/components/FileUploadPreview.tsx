...
import { X, Loader2, Github, Play, Music } from 'lucide-react';
...
export interface PendingFile {
  ...
  fileId?: string;
  fileName: string;
  fileType?: 'image' | 'document' | 'text' | 'video' | 'audio';
  mimeType: string;
  size: number;
  progress: number;
        ...
        {files.map((f) => {
          ...
          return (
            ...
            className={`relative group/file flex-shrink-0 w-28 h-28 rounded-xl border border-gray-200 hover:border-gray-300 dark:border-[#5B5B56] dark:hover:border-gray-400 overflow-hidden transition-all ${
              isImage || isVideo || isAudio ? '' : 'bg-white dark:bg-claude-input p-3 flex flex-col justify-between'
            }`}
          >
            {isGithub ? (
              <>
              ...
              <>
                <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-violet-50 to-purple-100 dark:from-violet-900/20 dark:to-purple-900/20">
                  <div className="w-10 h-10 rounded-full bg-violet-200 dark:bg-violet-800/50 flex items-center justify-center mb-1">
                    <Music size={18} className="text-violet-600 dark:text-violet-300" />
                  </div>
                  <div className="text-[10px] font-medium text-violet-600 dark:text-violet-300 truncate w-full text-center px-1" title={f.fileName}>
                    {f.fileName}
                  </div>
                </div>
                {f.status !== 'done' && (
