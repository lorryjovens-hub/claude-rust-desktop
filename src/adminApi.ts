...
const COSTS_BASE = '/api/costs';

function costsRequest(path: string, options: RequestInit = {}) {
  const token = getToken();
  const headers: HeadersInit = {
    'Content-Type': 'application/json',
    ...(options.headers || {}),
  ...
  return fetch(`${COSTS_BASE}${path}`, { ...options, headers }).then(res => {
    if (!res.ok) throw new Error(`Costs request failed: ${res.status}`);
    return res.json();
  });
}

export const getCostsDashboard = () => costsRequest('/dashboard');
export const getCostsBudget = () => costsRequest('/budget');
export const setCostsBudget = (data: { daily_budget?: number; monthly_budget?: number }) =>
  costsRequest('/budget', { method: 'POST', body: JSON.stringify(data) });
export const getCostsUsage = () => costsRequest('/usage');

export const setGlobalBudget = (data: { daily_budget?: number; monthly_budget?: number }) =>
  adminRequest('/budget/global', { method: 'POST', body: JSON.stringify(data) });
export const setUserBudget = (userId: string, data: { daily_budget?: number; monthly_budget?: number }) =>
  adminRequest(`/users/${userId}/budget`, { method: 'POST', body: JSON.stringify(data) });
export const getGlobalBudget = () => adminRequest('/budget/global');
export const getUserBudget = (userId: string) => adminRequest(`/users/${userId}/budget`);
