...
impl EnginePool {
    ...
    fn find_claude_engine_binary(&self) -> Result<PathBuf> {
        let candidates = if cfg!(target_os = "windows") {
            vec![
                ...
                PathBuf::from(r"C:\Users\user\.claude\bin\claude-haha"),
                PathBuf::from(r"C:\Users\user\.bun\share\claude\claude-haha"),
            ]
        } else {
            vec![
                self.workspace.join("engine").join("bin").join("claude-haha"),
                PathBuf::from(format!("{}/.claude/bin/claude-haha", std::env::var("HOME").unwrap_or_default())),
                PathBuf::from(format!("{}/.bun/share/claude/claude-haha", std::env::var("HOME").unwrap_or_default())),
            ]
        };
    ...
    pub async fn send_message(
        ...
        if let Some(handler) = self.message_handlers.get_mut(conv_id) {
            if let Some(output) = handler.recv().await {
                return Ok(output);
            }
        }
    ...
    pub async fn send_message_stream(
        ...
        Ok(self.message_handlers.remove(conv_id))
    }
